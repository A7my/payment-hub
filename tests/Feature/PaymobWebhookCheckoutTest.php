...
/**
 * End-to-end coverage of Paymob checkout confirmation:
 * - sdk  → Transaction Processed Callback (webhook) auto-confirms
 * - webview → package callback route confirms; webhook is ignored
 *
...
final class PaymobWebhookCheckoutTest extends TestCase
{
    ...
    /** @test */
    public function test_paymob_webhook_automatically_confirms_an_sdk_checkout_with_no_frontend_call(): void
    {
        ...
        // Sdk mode: webhook is the automatic confirmation path (webview uses
        // the package callback route instead — see sibling tests below).
    ...
    /** @test */
    public function test_paymob_webhook_does_not_confirm_a_webview_checkout(): void
    {
        $this->fakeHttp([
            '*/auth/tokens'             => [['token' => 'auth_1'], 200],
            '*/ecommerce/orders'        => [['id' => 555], 200],
        ...
        $pending = CheckoutTransaction::query()->where('model_type', 'order')->where('model_id', (string) $order->id)->first();
        $this->assertNotNull($pending);
        $this->assertSame('webview', $pending->driver_type);
    ...
    /** @test */
    public function test_paymob_callback_route_confirms_a_webview_checkout(): void
    {
        Event::fake([CheckoutPaymentConfirmed::class]);
        ...
        // Simulate Paymob's browser redirect (or dashboard Transaction Response
        // Callback) landing on the package callback route with the same flat
        // query shape as the Transaction Processed Callback.
        $query = $this->signedWebhookQuery($this->webhookPayload($pending->merchant_order_id));
        $this->get('/payment/checkout/callback/paymob?' . $query)->assertStatus(200);
        ...
        Event::assertDispatched(CheckoutPaymentConfirmed::class);
    }
